...
@author: Ayan Majumdar, Hui-Syuan Yeh, Chirag Bhuvaneshwara
"""
...
def halign(s,t):
    ...
    slen = len(s)
    tlen = len(t)    
    # Start off with worst score possible
    minscore = len(s) + len(t) + 1
    # Now align and get best score
...
def get_prefix_suffix_changes(lemma, inflection_form):
    ...
    suf_changes = []
    # Get all possible suffix changes
...
def generate_inflection(lemma, infl_rule, prefix_rules_dict, suffix_rules_dict):
    ...
    # These are possible endings that can come
    sufs = ['ar$','al$','in$','an$','il$','ul$','un$']
    end = base[-3:] 
    # Now go through every possible condition like a grammar table
    ...
    if infl_rule[0] == "V":
        ...
        if base.endswith("a$") and b in weak_class_1: 
    ...
    # We havent seen this, return original lemma
...
def main(argv):
    ...
    instances = 0
    train_file_lines = []
    train_rules_list = [] # Maintain a list of all inflection rules seen while training
    ...
    # Just adding more rules as was discovered from studying the grammar of the subject
    
    more_msds = ["N;INDF;NOM;SG", "N;INDF;ACC;SG", "N;INDF;DAT;SG", "N;INDF;GEN;SG", "N;INDF;NOM;PL", "N;INDF;ACC;PL", "N;INDF;DAT;PL", "N;INDF;GEN;PL",\
    "ADJ;NOM;MASC;SG", "ADJ;NOM;FEM;SG", "ADJ;NOM;NEUT;SG", "ADJ;ACC;MASC;SG", "ADJ;ACC;FEM;SG", "ADJ;ACC;NEUT;SG", "ADJ;DAT;MASC;SG", "ADJ;DAT;FEM;SG", "ADJ;DAT;NEUT;SG",\
    "ADJ;NOM;MASC;PL", "ADJ;NOM;FEM;PL", "ADJ;NOM;NEUT;PL", "ADJ;ACC;MASC;PL", "ADJ;ACC;FEM;PL", "ADJ;ACC;NEUT;PL", "ADJ;DAT;MASC;PL", "ADJ;DAT;FEM;PL", "ADJ;DAT;NEUT;PL",\
    "V;IND;PRS;1;SG", "V;IND;PRS;2;SG", "V;IND;PRS;3;SG", "V;IND;PRS;1;PL","V;IND;PRS;2;PL","V;IND;PRS;3;PL", "V;IND;PST;1;SG","V;IND;PST;2;SG","V;IND;PST;3;SG",\
    "V;IND;PST;1;PL","V;IND;PST;2;PL","V;IND;PST;3;PL", "V;IMP;2;SG", "V;IMP;2;PL"]
    msd_list = train_rules_list + more_msds
    # Make a set of the rules, to prevent duplicates
    # Then sort them 
    msd_list = list(set(msd_list))
    msd_list.sort()
    msd_list.sort(key = lambda s: len(s), reverse=True)
    ...
    for l in test_file_lines:
        if test_num_cols == 3:
            lemma, inflect_target, correct_rule, = l.split()
        elif test_num_cols == 2:
            lemma, inflect_target = l.split()
        lemmaorig = lemma
        ...
        for msd in msd_list:
            ...
            outform = generate_inflection(lemma, msd, prefix_rules_dict, suffix_rules_dict)
        
            #Reversing the reversed string before output
            if LANG_PREFIXED:
                outform = outform[::-1]
            if test_num_cols == 3:
                if outform == inflect_target and msd == correct_rule:
                    correct_preds += 1 # Correct if target inflection and the rule used match
                    if show_output==True:  
                        print(lemmaorig + "\t" + inflect_target + "\t" + correct_rule + "\n")
                    break
        test_instances += 1
